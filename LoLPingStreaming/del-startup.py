...
class App:
    ...
    def checkProcessRun(self, processName):
        '''
    ...
    def main(self, *args):
        self.isrun = "LoLPingStreaming.exe" in (p.name() for p in psutil.process_iter())
        gui.wlabel.config(text='')
        if not self.isrun:
            gui.label.config(text='The software is ready to Uninstall...')
            root.after(688*2, self.main2)
        else:
            gui.label.config(text='The software is running please terminate process (or force terminate using Ctrl+Shift+Esc)...')
            gui.wlabel.config(text='Retry')
            gui.wlabel.config(font='Helvetica 8 underline')
            gui.wlabel.config(fg='blue')
            gui.wlabel.config(cursor='hand2')
            gui.wlabel.bind('<Button-1>', self.main)

    def main2(self):
        try:
            gui.wlabel.config(text='')
            gui.wlabel.config(font='Helvetica 7 bold')
            gui.wlabel.config(fg='light blue')
            gui.wlabel.config(cursor='arrow')
            gui.wlabel.unbind('<Button-1>')
        except:
            pass
...
class Del:
    def __init__(self, root):
        ...
        self.title.pack(side='left', padx=(5, 0), anchor='w', pady=(8,0))
        ...
        self.label = Label(self.master, text='Checking if software is closed to perform Uninstall...', fg='light blue', bg='#000912', font='Helvetica 7 bold', bd=0)
